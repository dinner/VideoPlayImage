...
- (void)start
{
    ...
    if ([self isPrepared]) {
        if (!_timer) {
             ...
             */
            _timer = [NSTimer scheduledTimerWithTimeInterval:timeMargin target:self selector:@selector(getCurrentTime) userInfo:nil repeats:YES];
        }
...
- (void)resume
{
    if( ![self isPrepared] || _isPlaying == YES|| _isPauseing == NO) return;
